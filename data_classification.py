...
x, y = np.mgrid[-2:15:.01,-2:15:.01]
pos = np.dstack((x, y))
rv = multivariate_normal([mean1, mean2], cov_mtx)

fig, ax = plt.subplots(figsize=(8,8))
ax.contourf(x, y, rv.pdf(pos), cmap='YlOrBr')
sns.scatterplot(data=ps_std_df, x='ing_cor',y='energia',color='lightgreen',
                alpha=0.1, size=0.005, edgecolor=None)
ax.set_xlim((-2,15))
ax.set_ylim((-2,15))
...
fig, ax = plt.subplots(figsize=(8,8))
ax.contourf(x, y, rv.pdf(pos), cmap='YlOrBr')
sns.scatterplot(data=ps_std_df, x='alfabetism',y='energia',color='lightgreen',
                alpha=0.1, size=0.005, edgecolor=None)
ax.set_xlim((-2,15))
ax.set_ylim((-2,15))
ax.set_ylabel('Z score Energia (-)', fontsize=fontsize_ticks)
...
ax.tick_params(labelsize=fontsize_ticks)
sns.despine()
sns.scatterplot(data=ps_df, x='ing_cor',y='energia',ax=ax,color='blue', alpha=0.5,
               edgecolor=None)
ax2=ax.twiny()
ax2.tick_params(labelsize=fontsize_ticks)
sns.scatterplot(data=ps_df, x='alfabetism',y='energia',ax=ax2,color='orange',alpha=0.5,
               edgecolor=None)
...
ax.tick_params(labelsize=fontsize_ticks)
sns.despine()
sns.scatterplot(data=ps_std_df, x='ing_cor',y='energia',ax=ax,color='blue',
               edgecolor=None)
ax2=ax.twiny()
ax2.tick_params(labelsize=fontsize_ticks)
sns.scatterplot(data=ps_std_df, x='alfabetism',y='energia',ax=ax2,color='orange',
               edgecolor=None)
ax.set_xlim((-1,30))
ax2.set_xlim((-1,30))
ax.set_ylabel('Z score Energia (-)', fontsize=fontsize_ticks)
...
plt.savefig(sys_path+'/figures/Energia_Alfabetismp_Diferencias_Zscore.png', bbox_inches = 'tight',dpi=50)
plt.show()
