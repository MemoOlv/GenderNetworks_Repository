...
ENIGHr = pd.read_csv(sys_path+data_path+'ENIGH'+yr+'_clean.csv',
                         index_col=[0])
ENIGH = ENIGHr.copy()
...
R = dpf.ComputeRepresentativity(ENIGH,t,yr,sys_path+data_path)

#Read ENIGH covariance matrix energy
cov_matrixr = pd.read_csv(sys_path+data_path+'cov_matrix_'+t+yr+'.csv',
                         index_col=[0])
...
sns.despine()
sns.scatterplot(data=crt_p, x='cov', y='covmul',
                     alpha = 0.8, size='covmul', hue='covmul', ax=ax)
...
plt.savefig(sys_path+'/figures/RepCov_HAdultos.pdf', bbox_inches = 'tight')
plt.show()
...
ax.tick_params(labelsize=fontsize_ticks)
sns.despine()
sns.scatterplot(data=crt_p, x='idx', y=k,
                     alpha = 0.8, size='cov', hue='cov', ax=ax)
#sns.scatterplot(data=crt_pmax, x='idx', y=k, color='red',ax=ax)
for line in range(0,13):
    n_s = dpf.recur_fibo(line)
    plt.text(crt_p.idx[n_s],crt_p[k][n_s],crt_p.index[n_s],
...
plt.yscale('log')
plt.xscale('log')
plt.savefig(sys_path+'/figures/Representatividad_HAdultos.pdf', bbox_inches = 'tight')
plt.show()
...
ax = sns.lineplot(data=crt_sort, x='PC', y='value',
                  marker='o', label='Rep$_{m}$')
ax2=ax.twinx()
ax2.tick_params(labelsize=fontsize_ticks)
sns.lineplot(data=crt_sort, x='PC', y='num', ax=ax2,
             color='green',marker='o', label='No. personas')
...
plt.savefig(sys_path+'/figures/Rep_Pob_Hogares.pdf', bbox_inches = 'tight')
plt.show()
...
plt.savefig(sys_path+'/figures/CovMatrixCut'+yr+'.pdf', bbox_inches = 'tight')
plt.show()

cov_matrix_cut.to_csv(sys_path+data_path+'cov_matrix_'+t+yr+'_cut.csv',index=True)
