...
def condor_object(net):
    """Initialization of the condor object.
    The function gets a network in edgelist format encoded in a pandas dataframe.
    Returns a dictionary with an igraph network, names of the targets and regulators,
    list of edges, modularity, and vertex memberships.
    """
...
def initial_community(CO, method="LCS", project=False):
    ...
    The implementation using bipartite projection is not yet available,
    but project=False performs better modularity-wise (at least with the networks I worked on).
    """
    ...
    # Computes initial community assignement based on different methods.
    # Default method is Louvain clustering which is really fast.
    # The others take several times more.
    if not project:
        if method == "LCS":
    ...
    print(
        "Initial community structure computed in ",
        time.time() - t,
        ". Modularity = ",
        vc.modularity,
    )
...
def matrices(CO, c):
    ...
    # Index dictionaries for the matrix.
    # Note that this set of indices is different of that in the CO object
    # (that one is for the igraph network.)
    ...
    if "index" in CO["reg_memb"].columns:
        ed = zip(
            [rg[j] for j in [d[i] for i in CO["reg_memb"].iloc[:, 0]]], CO["reg_memb"].iloc[:, 1]
        )
    else:
