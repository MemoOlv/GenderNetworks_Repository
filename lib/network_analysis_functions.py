...
def network_construction(cov_matrix):
    ...
    network_pd = pd.DataFrame(
        list(zip(node1, node2, value)), columns=["Classification", "Variables", "Rep"]
    )

    RepHigh0 = list(
        network_pd[network_pd.Rep > 0][["Classification", "Variables", "Rep"]].itertuples(
...
def global_properties(Network, top_nodes, botm_nodes):
    ...
    mean_bet_node = round(
        np.mean(list(bipartite.centrality.betweenness_centrality(Network[0], top_nodes).values())),
        3,
    )
    mean_bet_edge = round(
        np.mean(
            list(
                nx.algorithms.centrality.edge_betweenness_centrality(
                    Network[0], weight="weight"
                ).values()
            )
        ),
        3,
...
def Community_Detection(Network):
    ...
    communities = [
        com_max_mod[len(com_max_mod) - 1][0][i]
        for i in range(0, len(com_max_mod[len(com_max_mod) - 1][0]))
    ]
...
def df_prop_nodes(Network, yr, CH):
    ...
    betweenness_lst = list(
        bipartite.centrality.betweenness_centrality(Network[0], CH).values()
    )  # betweenness

    dic_prop = {
        "Nodo": node_lst,
        "Fuerza": strenght_lst,
        "Clustering": clustering_lst,
        "Betweenness": betweenness_lst,
        "Grado": degree_lst,
    }
