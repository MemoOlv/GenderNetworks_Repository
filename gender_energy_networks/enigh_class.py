...
class ENIGH_Data(Data):
    ...
    def classification_sex_age(self, dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        # Sex classification
        sexHHRP = [(dataset.sexo_jefe == 1), (dataset.sexo_jefe == 2)]
        choices = ["H", "M"]
    ...
    def compute_representativity(self, covariance_matrix):
        ...
        for node in covariance_matrix.columns.unique():
            proportion = (
                (covariance_matrix[node] / sum(covariance_matrix[node]))
    ...
    def drop_nonessential_columns(self, dataset_merged):
        ...
        if self.year < 2018:
            dataset_merged.drop(columns=["ageb_x", "ageb_y"], inplace=True)

        dataset_merged.rename(
            columns={
    ...
    def read_data(self) -> pd.DataFrame:
        ...
        dataset_merged = reduce(
            lambda left, right: pd.merge(left, right, on="folioviv", how="outer"), datasets_list
        )
    ...
    def read_tables(self, table_name, year) -> pd.DataFrame:
        ...
        dataset = pd.read_csv(
            data_path + table_name, index_col="folioviv", low_memory=False, na_values=[" ", "&"]
        )
        return dataset
        ...
        for node in dataset.node.unique():
            ...
            for column in dataset_node.columns[:-1]:
                if dataset_node[column].std(ddof=0) == 0:
                    ...
                    dataset_node[column] = (
                        dataset_node[column] - dataset_node[column].mean()
                    ) / dataset_node[column].std()
